...
}// it will render the image coming from server side
...
function currencyFmt(amount = 0, currency = "INR") {// this function give the amount either in inr or in usd
  try {
    if (currency === "INR") {
...
//compute total for subTotal tax and total value 
...
export default function CreateInvoice() {
  ...
  //upload image and review it also can remove after review
  ...
  async function handleSave() {
    ...
    try {
      ...
      const prepared = {
        ...
        notes: invoice.notes || "",
        localId: invoice.id,
      };// these all details will save in DB in backend
      ...
      const endpoint =
        isEditing && invoice.id
          ? `${API_BASE}/api/invoice/${invoice.id}`     //put method to update any invoice 
          : `${API_BASE}/api/invoice`;              // post route to create any invoice
