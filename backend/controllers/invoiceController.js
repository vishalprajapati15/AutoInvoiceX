...
}// compute total, tax and subtotal
...
function parseItemsField(val) {
    ...
    if (typeof val === "string") {
        try {
            ...
            console.log("Error in handling value from parse item field")
            return []
        }
    }
    return val;
...
function uploadFilesToUrls(req) {
    const urls = {};
    if (!req.files) {
        return urls;
    }
...
export async function createInvoice(req, res) {
    try {
        ...
        let invoiceNumberProvided = typeof body.invoiceNumber === "string" && body.invoiceNumber.trim()
            ? String(body.invoiceNumber).trim()
            : null;
        //invoicenumber if present then error else ok
        ...
        let saved = null;
        let attempts = 0;
        const maxSaveAttempt = 6;
        while (attempts < maxSaveAttempt) {
            try {
                saved = await doc.save();
                break; //succes
            } catch (err) {
                //if duplicate invoiceNumber the regenerate and retry
                if (err && err.code === 11000 && err.keyPattern && err.keyPattern.invoiceNumber) {
                    attempts += 1;
                    //generate new invoice number and set to doc
                    const newNumber = await generateUniqueInvoiceNumber();
                    doc.invoiceNumber = newNumber;
                    continue; // loop to try save again
                }
                throw err;
            }
        }
        ...
        //handle duplicate key at top-level
        if (err && err.code === 11000 && err.keyPattern && err.keyPattern.invoiceNumber) {
...
export async function getInvoices(req, res) {
    try {
        ...
        if (req.query.search) {
            ...
            q.$or = [
                { forEmail: { $regex: search, $options: "i" } },
...
export async function updateInvoice(req, res) {
    try {
        ...
        const update = {
            ...
            client:
                typeof body.client === "string" && body.client.trim()
                    ? { name: body.client } : body.client || existing.client || {},
            items,
            subtotal: totals.subtotal,
            tax: totals.tax,
            total: totals.total,
...
//to send invoice via email with simplw summary 
export async function sendInvoiceEmail(req, res) {
    try {
        ...
        //Fetch the invoice from database to verify ownsership
        const query = isObjectIdString(invoiceId)
            ? { id: invoiceId, owner: userId }
            : { invoiceNumber: invoiceId, owner: userId }
        ...
        await sendEmail({
            email,
            subject: subject || `Invoice ${invoice.invoiceNumber || invoice._id} from Invoice AI`,
            message: message || `Please find your invoice details below: \n\nInvoice Number ${invoice.invoiceNumber}\nTotal: ${invoice.total}\nDue Date: ${invoice.dueDate}\n\nThank you for your business!`
        });

        return res.status(200).json({
            success: true,
            message: `Invoice details sent successfully to ${email}`
        });
